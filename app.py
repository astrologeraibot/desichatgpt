    ...
    with st.chat_message("assistant"):
        ...
        if user_input.lower() == "/start":
            ...
            try:
                ...
                match = re.search(
                    r"(?P<date>(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})|(\d{4}-\d{2}-\d{2})|(\w+ \d{1,2},? \d{4}))"
                    r"[ ,]*at[ ,]*"
                    r"(?P<hour>\d{1,2}):?(?P<minute>\d{0,2})[ ]?(?P<ampm>[apAP][mM])?[ ,]*in[ ,]*(?P<place>.+)",
                    user_input
                )
                ...
                if match:
                    ...
                    ampm = groups.get("ampm", "").lower()
                    place = groups["place"].strip()
                    ...
                    response = f"✅ Birth details recorded:\n\n📅 {birth_date}, 🕒 {birth_time}, 📍 {place}\n\nAsk me your horoscope, sun sign, or love prediction!"
                else:
                    response = "❌ I couldn't read your full birth details. Try something like:\n\n`I was born on 21 July 1995 at 9:00 AM in Delhi`"
            ...
            response = "🤔 I'm not sure what you mean. Try asking for your horoscope, sun sign, or love prediction."
